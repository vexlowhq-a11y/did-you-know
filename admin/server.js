/*
  Panel de administración de Did You Know? — servidor local
  =====================================================
...
// "History of the World Champions"). Mismo patrón que listTopics/
// topicThumbPath, una carpeta de imagen más adentro.
...
var server = http.createServer(function (req, res) {
  ...
  if (urlPath === '/api/categories' && req.method === 'POST') {
    return readBody(req, function (err, data) {
      ...
      try {
        var created = pagegen.addCategory(data.label, data.icon);
  ...
  if (urlPath === '/api/categories' && req.method === 'PATCH') {
    return readBody(req, function (err, data) {
      if (err || !data || !data.slug) {
        return sendJSON(res, 400, { error: 'Falta el slug de la categoría' });
      }
      try {
        var updated = pagegen.updateCategory(data.slug, { label: data.label, icon: data.icon });
        return sendJSON(res, 200, { ok: true, category: updated });
      } catch (e) {
        return sendJSON(res, 400, { error: e.message });
      }
    });
  ...
  if (urlPath === '/api/categories' && req.method === 'DELETE') {
    return readBody(req, function (err, data) {
      ...
      var usedByArticles = articles.filter(function (a) { return a.category === slug; });

      var hero = [];
      try { hero = readJSON(path.join(DATA_DIR, 'hero.json')); } catch (e) { hero = []; }
      var usedByHero = hero.filter(function (s) { return s.category === slug; });

      var topicGroups = pagegen.loadTopicGroups();
      var topicCount = (topicGroups[slug] || []).reduce(function (n, g) { return n + g[1].length; }, 0);

      if (usedByArticles.length || usedByHero.length || topicCount) {
        var parts = [];
        if (usedByArticles.length) parts.push(usedByArticles.length + ' artículo(s)');
        if (usedByHero.length) parts.push(usedByHero.length + ' diapositiva(s) del carrusel');
        if (topicCount) parts.push(topicCount + ' tema(s)');
        return sendJSON(res, 409, {
          error: 'Esta categoría todavía tiene ' + parts.join(', ') + '. Movelos o eliminalos antes de borrar la categoría.',
          articles: usedByArticles.map(function (a) { return a.title; })
        });
      }
...
server.listen(PORT, function () {
  console.log('');
  console.log('  Did You Know? — Panel de administración');
